...
import (
	...
	metricsclient "k8s.io/kubernetes/pkg/controller/podautoscaler/metrics" // metrics
...
var (
	...
	// errSpec được sử dụng để quyết định nếu lỗi đến từ HPA object trong reconcileAutoscaler
	// Tất cả các lỗi như thế này nên được ném ra ở root để các hàm có thể được loại lỗi đó đến từ đâu và phân biệt các lỗi.
	// e.g., fmt.Errorf("invalid spec%w", errSpec)
	errSpec error = errors.New("")
)

// Struct này được dùng để lưu trữ khuyến nghị autoscaling
type timestampedRecommendation struct {
	recommendation int32     // số lượng replicas được tăng lên hoặc giảm xuống
	timestamp      time.Time // thay đổi replicas sẽ xảy ra vào thời gian nào
}

// Struct này được dùng để lưu trữ sự kiện khi scale
type timestampedScaleEvent struct {
	replicaChange int32     // là một giá trị không âm
	timestamp     time.Time // thay đổi vào thời gian nào
	outdated      bool      // sự kiện này đã lỗi thời/hết hiệu lực hay chưa
}
...
// struct này chịu trách nhiệm đồng bộ các HPA object được lưu trữ trong hệ thống với
// số lượng deployment/replications controller mà chúng quản lý (các controller quản lý)
type HorizontalController struct {
	scaleNamespacer scaleclient.ScalesGetter                         // lấy thông tin về số lượng bản sao scale của các đối tượng
	hpaNamespacer   autoscalingclient.HorizontalPodAutoscalersGetter // lấy thông tin và cập nhật các pod Autoscaler
	mapper          apimeta.RESTMapper                               // cung cấp khả năng ánh xạ từ Restful API tới tài nguyên nội bộ

	replicaCalc   *ReplicaCalculator   // thành phần tính toán số lượng replicas cần thiết dựa trên các metrics thu thập được
	eventRecorder record.EventRecorder // lưu lại các sự kiện đã xảy ra trong hệ thống

	downscaleStabilisationWindow time.Duration // thời gian còn lại để tiến hành scaledown

	monitor monitor.Monitor // được sử dụng để giám sát metrics từ bên ngoài

	// hpaLister is able to list/get HPAs from the shared cache from the informer passed in to
	// NewHorizontalController.

	// hpaListener có khả năng lấy danh sách HPAs từ bộ nhớ đệm chia sẻ để tạo một NewHorizontalController
	hpaLister       autoscalinglisters.HorizontalPodAutoscalerLister // Được sử dụng để liệt kê và truy vấn các đối tượng HPA từ bộ nhớ đệm chia sẻ.
	hpaListerSynced cache.InformerSynced                             // kiếm tra dữ liệu về HPA trong bộ nhớ đệm đã được đồng bộ hóa hay chưa

	// podLister is able to list/get Pods from the shared cache from the informer passed in to
	// NewHorizontalController.

	// podListener có khả năng lấy danh sách Pods từ bộ nhớ đệm để tạo một NewHorizontalController
	podLister       corelisters.PodLister // Được sử dụng để liệt kê và truy vấn các đối tượng Pod từ bộ nhớ đệm chia sẻ.
	podListerSynced cache.InformerSynced  // Kiểm tra xem liệu bộ nhớ đệm Pod đã được đồng bộ hóa hay chưa.

	// Controllers that need to be synced
	// controller cần được đồng bộ
	queue workqueue.TypedRateLimitingInterface[string] // Là hàng đợi các đối tượng cần được đồng bộ hóa bởi Controller, sử dụng cơ chế giới hạn tốc độ.

	// Latest unstabilized recommendations for each autoscaler.
	// Khuyến nghị mới nhất  cho từng autoscaler
	recommendations     map[string][]timestampedRecommendation // Lưu trữ các khuyến nghị gần đây nhất về số lượng bản sao cho mỗi autoscaler.
	recommendationsLock sync.Mutex                             // bảo vệ truy cập đến map recommendations mà không bị xảy ra tình trạng xung đột trong môi trường đa luồng

	// Latest autoscaler events
	// lưu trữ các sự kiện scale gần nhất
	scaleUpEvents       map[string][]timestampedScaleEvent // Lưu trữ các sự kiện tăng số lượng bản sao gần đây cho mỗi autoscaler.
	scaleUpEventsLock   sync.RWMutex                       // Đảm bảo an toàn cho các hoạt động đọc và ghi trên scaleUpEvents trong môi trường đa luồng.
	scaleDownEvents     map[string][]timestampedScaleEvent // Lưu trữ các sự kiện giảm số lượng bản sao gần đây cho mỗi autoscaler.
	scaleDownEventsLock sync.RWMutex                       // Đảm bảo an toàn cho các hoạt động đọc và ghi trên scaleDownEvents trong môi trường đa luồng.

	// Storage of HPAs and their selectors.
	hpaSelectors    *selectors.BiMultimap // Lưu trữ các selector của HPA và đối tượng được HPA quản lý.
	hpaSelectorsMux sync.Mutex            // Đảm bảo rằng các hoạt động trên hpaSelectors không gặp phải xung đột trong môi trường đa luồng.
}
...
func NewHorizontalController(
	...
	// đăng kí một recorder
	broadcaster := record.NewBroadcaster(record.WithContext(ctx))
	broadcaster.StartStructuredLogging(3)
	broadcaster.StartRecordingToSink(&v1core.EventSinkImpl{Interface: evtNamespacer.Events("")})
	recorder := broadcaster.NewRecorder(scheme.Scheme, v1.EventSource{Component: "horizontal-pod-autoscaler"})

	// thiết lập một horizontalcontroller để lưu lại các sự kiện (eventRecorder)
	// hoặc lấy thông tin bản sao scale của đối tượng
	// và một hàng đợi (queue) để để quản lý công việc cần thực hiện
	// và các sự kiện scale cùng các mutex để đảm bảo nhất quán trong môi trường đa luồng.
	...
	// đăng kí các hàm xử lý sự kiện (Thêm, Sửa, xóa) các HPA để controller phản ứng lại với thời gian
	// đồng bộ (resyncPeriod)
	...
	// Tạo một ReplicaCalculator, thành phần này chịu trách nhiệm tính toán số lượng replicas
	// (Pod) cần thiết dựa trên các số liệu thu thập được.
...
func (a *HorizontalController) Run(ctx context.Context, workers int) {
	...
	defer utilruntime.HandleCrash() // Xử lý bất kì panic nào xảy ra
	defer a.queue.ShutDown()        // khi chạy xong thì shutdown hoàn toàn

	logger := klog.FromContext(ctx)                   // sử dụng để ghi log
	logger.Info("Starting HPA controller")            // Show log lúc bắt đầu chạy
	defer logger.Info("Shutting down HPA controller") // show ra khi kết thúc

	if !cache.WaitForNamedCacheSync("HPA", ctx.Done(), a.hpaListerSynced, a.podListerSynced) {
		//  Đợi cho đến khi bộ nhớ đệm (cache) của hpaLister và podLister được đồng bộ hóa.
		// Nếu bộ nhớ đệm không đồng bộ hóa thành công (ví dụ như do lỗi mạng hoặc các vấn đề khác),
		// controller sẽ dừng lại và không tiếp tục chạy.
		// Điều này đảm bảo rằng controller chỉ hoạt động khi nó có thông tin chính xác từ bộ nhớ đệm.
		return
	}

	for i := 0; i < workers; i++ {
		go wait.UntilWithContext(ctx, a.worker, time.Second)
		// khởi chạy các workder dưới một goroutine riêng biệt, mỗi worker sẽ thực hiện
		// công việc trong hàng đợi với tần suất mỗi GIÂY một lần
	}

	<-ctx.Done() // controller sẽ chạy liên tục cho đến khi context được hủy bỏ hoặc kết thúc
}

// obj could be an *v1.HorizontalPodAutoscaler, or a DeletionFinalStateUnknown marker item.
// cập nhật HPA, thêm cur vào a ???
...
func (a *HorizontalController) enqueueHPA(obj interface{}) {
	// tạo ra key duy nhất dựa trên namespace cho Obj
	...
	// key vừa được tạo ra ở trên sẽ được thêm vào hàng đợi với một độ trễ là chu kỳ đồng bộ (resyncPeriod)
	// Nếu có một request cho HPA trong hàng đợi rồi thì request mới sẽ không được thêm
	// điều này đảm bảo cho HPA xử lý yêu cầu với đúng chu kì của nó
...
func (a *HorizontalController) deleteHPA(obj interface{}) {
	...
	a.queue.Forget(key) // xóa key của obj trong hàng đợi để không xử lý yêu cầu của obj này nữa
...
func (a *HorizontalController) worker(ctx context.Context) {
	// vòng lặp xử lý liên tục các công việc trong hàng đợi
...
// computeReplicasForMetrics tính toán số lượng replicas cho những metrics được liệt kê trong HPA
// hàm trả về số lượng replicas tối đa, mô tả cho các metrics và trạng thái của các metrics được tính toán ???
// hàm có thể trả về cả số lượng replica mong muốn và lỗi
// Nếu một số metrics vẫn ok thì nó sẽ scale theo các metrics đó
// nếu không thể scale, HPA sẽ return -1 cho số lượng replica mong muốn.
func (a *HorizontalController) computeReplicasForMetrics(ctx context.Context, hpa *autoscalingv2.HorizontalPodAutoscaler, scale *autoscalingv1.Scale,
	...
	specReplicas := scale.Spec.Replicas                             // số lưởng replica hiện tại
	statusReplicas := scale.Status.Replicas                         // trạng thái các replica hiện tại
	statuses = make([]autoscalingv2.MetricStatus, len(metricSpecs)) // tạo một mảng tương ứng với số lượng các metrics
	...
	for i, metricSpec := range metricSpecs {
		// tính toán số lượng replica mong muốn cho từng metrics
		...
		// Nếu ban đầu chưa có replica nào hoặc số lượng replica sau tính toán lớn hơn hiện tại thì cập nhật
		// các metrics khác nhau vẫn lấy số lượng các replicas lớn nhất
